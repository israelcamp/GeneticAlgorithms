...
class Population():
    ...
    def _Changing(self, previous_max_fit, number_gen_btw):
        max_fit, _ = self.MaxFitness()
        if abs(max_fit - previous_max_fit) > 0.00001:
    ...
    def EvolvePop(self):
        ...
        self.number_gen_btw = 0
        count_gen = 0
        while self._Evolving() and self._Changing(self.previous_max_fit, self.number_gen_btw) < 200:
            max_fit, best_member = self.MaxFitness()
            if self.number_gen_btw == 0:
                print("Function Evaluation: {} -- Point {} -- Gen {}".format(max_fit, best_member.genes, count_gen))
